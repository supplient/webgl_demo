 ...
 */
export function loadFileSource(filename_list) {
    var file_loaders = [];
    ...
    for(var i=0; i<filename_list.length; i++){
        ...
        file_loaders.push(new Promise(function(resolve, reject){
            ...
            // TODO raise error if file load failed
 ...
 */
export async function loadProgram(gl, vshaderfile, fshaderfile) {
 ...
 * Load shaders
 * @param gl WebGL context
 * @param shader_list a list of shaer files. Each of the element is a list like [vert_shader, frag_shader].
 * @return a Promise. Its then will receive a list of program.
 */
export function loadPrograms(gl, shader_list) {
    var load_list = [];
    for (const shader_files of shader_list)
        load_list.push(loadProgram(gl, shader_files[0], shader_files[1]));
    return Promise.all(load_list);
}
